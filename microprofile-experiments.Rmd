...
## Microprofile temperature corrections
...
We will start by pulling out the last three observations from each profile, assuming that these values represent 'true' zeros. These data points are then merged with the corresponding HOBO temperature log data to assess how the baseline signal changes with temperature.
```{r}
# Process the tibble
...
df_sliced %>% 
  ...
  pull(temp_correct)
```
PROFILE CORRECTION NEEDS TO BE FINISHED.
...
df_combined <- df_combined %>%
  mutate(profiles = map2(profiles, cals, ~ {
    # Group by the variables within the nested dataframe
    .x %>%
      group_by(date, incubation, experiment, light.dark, treatment, flume, core) %>%
      # Use the slope and intercept from the matching calibration data
      mutate(concentration = (.x$mV - .y$intercept) / .y$slope)
  }))
```
