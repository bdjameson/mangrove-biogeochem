...
df_corrected <- df_combined %>%
  plyr::mutate(profiles = map2(profiles, df_model$drift_model, ~ .x %>%
    ...
    select(-time_slope, -temp_slope, -time_significant, -temp_significant, -time_p_value, -temp_p_value, -time_numeric) # Clean up
  ))
```
...
Now that the profiles have been corrected for signal drift, we can apply the calibration coefficients to calculate analyte concentrations. We will also attempt to identify the "best" calibration based on profile bottom values closest to zero. 
```{r}
df_calibrated <- df_corrected %>%
  mutate(profiles = map2(profiles, cals, ~ {
    ...
    # Separate calibration data into distinct datasets for cal.ID 1 and 2
    cal1 <- .y %>%
      filter(cal.ID == 1) %>%
    ...
    # Apply calibration logic for each profile
    profiles_with_cal <- profiles_with_cal %>%
      group_by(experiment, light.dark, channel, analyte, flume, core) %>%
      mutate(
        # Calculate concentrations for both calibrations
        concentration1 = if_else(
          !is.na(intercept1) & !is.na(slope1), 
          (corrected_mV - intercept1) / slope1, 
          NA_real_
        ),
        concentration2 = if_else(
          !is.na(intercept2) & !is.na(slope2), 
          (corrected_mV - intercept2) / slope2, 
          NA_real_
        ),
        # Select the best calibration for non-H2S analytes
        final_concentration = if_else(
          analyte != "H2S",
          case_when(
            !is.na(concentration1) & (is.na(concentration2) | abs(concentration1) <= abs(concentration2)) ~ concentration1,
            !is.na(concentration2) & (is.na(concentration1) | abs(concentration2) < abs(concentration1))  ~ concentration2,
            TRUE ~ NA_real_
          ),
          # Use only concentration1 for H2S analytes
          concentration1
        )
      ) %>%
      ungroup() %>%
  ...
  mutate(
    profiles = map(profiles, ~ {
      .x %>%
        ...
        dplyr::mutate(
          mean_bottom_three = mean(head(concentration1, 3)),  # Take the top 3 rows after sorting
          adjusted_value = concentration1 - mean_bottom_three  # Subtract the mean from concentration1
        ) %>%
        ungroup()
    })
  )
