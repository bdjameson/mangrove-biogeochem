...
#
# 🗂️ Metadata and HOBO Logger Data
...
### 🧩 Map HOBO data to metadata table
...
### ✂️ Trim HOBO Logger Data to Incubation Intervals
...
Before proceeding with calibration and correction, it’s important to visually confirm that the **temperature records** align with the defined incubation periods.  
...
- 🟦 **Dark incubation start**
- 🟩 **Light incubation start**
- 🟥 **End of incubation**
...
# 🧪 Profiles and Calibrations

This section imports and organizes **microelectrode profile data** and their corresponding **sensor calibrations**.  
Microprofiles for O₂, N₂O, NO, and H₂S were collected using Clark-type microelectrodes (Unisense) during both light and dark incubations.  
...
Microprofiles (O₂, N₂O, NO, H₂S) are imported and structured into **nested data frames** for each experiment.  
...
# ✅ Quick check: confirm structure and units
glimpse(cal_trim)
```
...
Before applying corrections or calibrations, it’s important to visually inspect the **raw sensor data** for consistency and potential anomalies.

This step helps identify outlier signals, noisy sensors, or incomplete profiles prior to quantitative processing.

## 📊 Exploratory QC Plots (Raw Data)
...
🧩 Note:
Selection criteria can be customized — for example, retaining the replicate with the highest signal strength, most complete depth coverage, or least noise.

## ⚗️ Addressing Sulfide Interference
...
- ✅ Profiles retained: clean signals approaching zero near the sediment base.

- ⚠️ Profiles removed: likely contaminated by H₂S cross-sensitivity.

- 💡 Removing these early ensures the drift model assumptions remain valid and improves downstream correction accuracy.
...
- Smooth signal decay toward the sediment base (for NO, N₂O, O₂)  
- Stable, non-spiking H₂S profiles
...
Drift arises from gradual shifts in the electrode’s baseline potential, often due to small changes in temperature, electrode polarization, or electronic instability during measurement.

## 🌡️ Map Incubation Temperatures to Microprofile Observations
...
Next, we extract the bottom three readings from each profile — assumed to represent true zero-analyte conditions (following Jameson et al., 2024).
...
Before applying any statistical correction, it’s important to **visualize how the sensor baseline changes** through time and with temperature.  
This diagnostic step helps identify whether drift is primarily **time-dependent**, **temperature-driven**, or minimal altogether.

We’ll generate exploratory plots showing how the **zero-point voltages (mV)** vary across each experiment and analyte.  
...
When measurement temperatures are stable and the experimental duration is relatively short (typically less than 4 hours), sensor drift can often be modeled as a simple linear function of time. This correction assumes that drift arises from gradual changes in the sensor’s electrochemical baseline—such as slow reference electrode potential shifts or membrane stabilization—rather than external temperature fluctuations.

In this step, a linear regression model (mV ~ time) is fitted independently for each analyte and channel using only the zero-domain points (i.e., the bottom few measurements where true analyte concentration is minimal). The model estimates a baseline drift slope (ΔmV/sec) and tests its statistical significance.

For each experiment:

- ✅ Only slopes with p < 0.05 are retained for correction (adjust for more stringent correction procedure).

- ⚙️ The initial calibration intercept (from the pre-run calibration) is used as the reference baseline.

- 🚫 Profiles without significant slopes are left uncorrected, ensuring that random variation is not overfitted.
...
For each analyte–channel group:

- 📊 Models are retained only if adjusted R² ≥ 0.3 and at least one predictor is significant (p < 0.01).

- 🧠 If both time and temperature effects are significant, a combined correction is applied; otherwise, only the relevant term is used.

- 🔒 If neither is significant, the original signal is preserved.
...
Once the sensor drift has been corrected, the next step is to **convert voltage (mV) readings into analyte concentrations (µmol L⁻¹)** using the experimentally derived calibration curves.  

Calibration ensures that each sensor’s electrical response is properly scaled, allowing direct comparison across treatments, analytes, and experiments.
...
- If **slope and intercept values remain stable** (≤10% change), we apply the *first calibration* (pre-run).  
...
📊 Inspect Calibrated Profiles

After applying the calibrations, we can visualize the converted profiles to confirm that each analyte’s concentration gradient behaves as expected.
...
For most analytes (O₂, N₂O, NO), we assume concentrations at the *bottom of the profile* are close to zero.  
For **H₂S**, this assumption is reversed — we expect near-zero concentrations at the *top of the profile* due to oxidation near the sediment–water interface.
...
- ✅ Profiles land near zero at expected boundary → baseline successfully standardized

- ⚠️ Offsets remain (>5 µmol L⁻¹) → recheck calibration or replicate assignment

- 🌀 H₂S shows inversion → confirm top-down orientation of depth sorting
